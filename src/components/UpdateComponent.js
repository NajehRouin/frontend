...
const  UpdateComponent=({
    ...
    <div className="mx-auto bg-white shadow-md p-4 w-full max-w-sm rounded-lg">
             ...
             <div className='flex justify-between items-center pb-3'>
                 <h2 className='font-bold text-lg'>Modifier Component</h2>
           ...
           <form className='grid p-4 gap-2 overflow-y-scroll h-full pb-5' onSubmit={handleSubmit}>
             ...
             <label htmlFor='Prix' className='mt-3'>Prix :</label>
             <input 
               type='number' 
               id='Prix' 
               placeholder='Prix' 
             ...
             />
               ...
               <label htmlFor='matiere' className='mt-3'>Materiel :</label>
               <select required value={matierleComponent} name='matiere' onChange={handleChangeMateriel} className='p-2 bg-slate-100 border rounded'>
                   <option value={""}>Select Materiel</option>
                   {
                     materiel.map((el,index)=>{
                       return(
                         <option value={el._id} key={el._id}>{el.nom}</option>
                       )
                     })
                   }
               </select>
               <button className='px-3 py-2 bg-red-600 text-white mb-10 hover:bg-red-700'>Modifier Component</button>
           </form> 
